...
class DQNAgent:
    def __init__(self, state_size, action_size, device, learning_rate=1e-4, gamma=0.99, epsilon=1.0, epsilon_min=0.01, epsilon_decay=0.0005, memory_size=2000):
        # Initialize DQNAgent with parameters
    ...
    def update_target_network(self):
        # Update the target network's weights with the Q-network's weights
        self.target_network.load_state_dict(self.q_network.state_dict())

    def remember(self, state, action, reward, next_state, done):
        # Remember an experience in the replay memory
        self.memory.append((state, action, reward, next_state, done))

    def replay(self, batch_size):
        # Sample a batch of experiences from the replay memory and train the Q-network
    ...
    def save_model(self, filename):
        # Save the Q-network's weights to a file
        torch.save(self.q_network.state_dict(), filename)

    def load_model(self, filename):
        # Load the Q-network's weights from a file
